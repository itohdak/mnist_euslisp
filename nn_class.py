...
class MLP():
    ...
    def train(self, x, t, learning_rate, momentum_rate):
        ...
        print "self.y[np.where(t == 1)]:\n{}\n\n".format(self.y[np.where(t == 1)])
        ...
        for layer in self.layers:
            ...
            print "len(z):\n{}\nlayer.delta.shape:\n{}\ndb.shape:\n{}\n\n".format(len(z), layer.delta.shape, db.shape)
...
n_neuron = 20
sample = '''
model = MLP([Perceptron(50, n_neuron, 1.0, ReLU),
             Perceptron(n_neuron, n_neuron, 1.0, ReLU),
             Perceptron(n_neuron, 10, 1.0, Softmax)])
model.train(x, t, lr, 0)
'''
x = np.arange(10).reshape((-1,10)) * (1. / 9.)
t = np.eye(10)[0].reshape((-1,10))
print "x:\n{},\nt:\n{}".format(x.shape, t.shape)
# print sample
model = MLP([Perceptron(10, n_neuron, 1.0, ReLU),
             Perceptron(n_neuron, n_neuron, 1.0, ReLU),
             Perceptron(n_neuron, 10, 1.0, Softmax)])
print model.train(x, t, lr, 0)
